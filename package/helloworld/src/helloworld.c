...
void * write_socket(int speed,int newfd);
void jsonStringGenerator(int speed_input,int charge_input,int current_input,char mystring []);
...
int main(int argc, char** argv)
{
    ...
    printf("received data:%s\n from %s\n",buf,inet_ntoa(client[i].addr.sin_addr)); 
     write_socket(autoincreasespeed++,sockfd);   
                        }                
                        else
                        {
...
void * write_socket(int speed,int 
 newfd )
{
        ...
        jsonStringGenerator(speed,80,1,mesg_str);
...
void jsonStringGenerator(int speed_input,int charge_input,int current_input,char mystring [])
{
  ...
  sprintf(mystring,"{\"speed\":%d,\"charge\":%d,\"current\":%d}$\n",speed_input,charge_input,current_input);
